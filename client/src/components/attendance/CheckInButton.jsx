...
const CheckInButton = ({ todayRecord, onAction }) => {
  ...
  if (todayRecord?.checkOut) {
    return (
      <div className='flex flex-col items-center justify-center p-8 bg-slate-50 rounded-2xl border border-slate-200'>
        ...
        <p className='text-slate-500 text-sm mt-1'>Great job! se you tomorrow</p>
      </div>
    )
  }
  ...
  const isCheckedIn = !!todayRecord?.isCheckedIn
  // const isCheckedIn = Boolean(todayRecord?.isCheckedIn)
  ...
  return (
    <div className='absolute bottom-4 right-4 flex flex-col z-10'>
      ...
      >
        {loading ?
          <Loader2Icon className='size-7 animate-spin' /> : isCheckedIn ? <LogOutIcon className='size-7' /> : <LogInIcon className='size-7' />
        }

        <div className='relative flex flex-col items-center text-center'>
          <h2 className='text-lg font-medium mb-1'>{loading ? "Processing" : isCheckedIn ? "Clock Out" : "Clock In"}</h2>
          <p className='text-xs opacity-80'>{isCheckedIn ? "Click to end your shift" : "Start you work day"}</p>
        </div>
      </button>
    </div>
  )
