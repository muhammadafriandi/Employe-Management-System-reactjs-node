              ...
              leaves.map((leave) => {
                return (
                  <tr key={leave._id || leave.id}>
                    ...
                    <td className='badge bg-slate-100 text-slate-600'><span>{leave.type}</span></td>
                    ...
                    <td>
                      <span className={`badge ${leave.status === "APPROVED" ? "badge-success" : leave.status === "REJECTED" ? "bagde-danger" : "badge-warning"}`}>
                        {leave.status}
                      </span>
                    </td>
                    {isAdmin && (
                      <td>{leave.status === "PENDING" && (
                        <div className='flex justify-center gap-2'>
                          ...
                          >
                            {processing === (leave._id || leave.id) ? <Loader2 className='w-4 h-4 animate-spin' /> : <X />}
                          </button>
                        </div>
                      )}</td>
                    )}
