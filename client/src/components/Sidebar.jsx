...
const Sidebar = () => {
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              />
              <span className="felx-1">{item.name}</span>
              {isActive && (
  ...
  return (
    <>
      ...
      {/* Sidebar Mobile */}
      <aside
        className={`lg:hidden fixed inset-y-0 left-0 w-72 bg-linear-to-b from-slate-900 via-slate-900 to-slate-950 text-white z-50 flex flex-col transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        {SidebarContent}
      </aside>
    </>
